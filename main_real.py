...
if __name__ == "__main__":
    ...
    for i, dataBatch in enumerate(brdfLoader):
        ...
        # newmasks = torch.zeros_like(seg1Batch)
        # newmasks[torch.linspace(0,opt.camNum - 1,opt.camNum).reshape(opt.camNum,1).repeat((1,new_y.size(1))).long(),torch.zeros_like(new_y).long(),new_y,new_x] = 1
        ...
        voxel_res_list = [32, 40, 48, 56, 64, 96,128, 256]
        ...
        chamfer_dis = CalChamferDis(osp.join(shapePath, 'visualHullSubd_%d.ply' % (opt.camNum)),
                                    osp.join(shapePath, 'meshGT_transform.ply'))
        ...
        # mask_gt = torch.abs(image_bg - imBgBatch)
        # mask_gt = (torch.sum(mask_gt,dim=1,keepdim=True) > 0.5).float()
        #
        ...
        while (grid_res_x <= 256):
            ...
            # image_initial = (torch.clamp(refractImg2, 0, 1)).data.permute(0, 3, 1, 2)
            ...
            torchvision.utils.save_image(imBgBatch,
                                         ...
                                         scale_each=False, pad_value=0)
            # torchvision.utils.save_image(torch.abs(image_initial - imBgBatch),
            ...
            #optimizer = torch.optim.SGD([grid_initial], lr=learning_rate)
            #optimizer = torch.optim.RMSprop([grid_initial], lr=learning_rate, eps=1e-8)
            watch_grid = grid_initial.data.cpu().numpy()
            watch_gt = grid_gt.data.cpu().numpy()

            if grid_res_z == opt.gridsize and opt.maskloss > 0:
                coord_x = torch.linspace(0, grid_res_x - 1, grid_res_x).cuda() * voxel_size + bounding_box_min_x
                coord_y = torch.linspace(0, grid_res_y - 1, grid_res_y).cuda() * voxel_size + bounding_box_min_y
                coord_z = torch.linspace(0, grid_res_z - 1, grid_res_z).cuda() * voxel_size + bounding_box_min_z
                grid_x,grid_y,grid_z = torch.meshgrid([coord_x,coord_y,coord_z])
                ...
                grid_bs = torch.linspace(0,opt.camNum - 1, opt.camNum).reshape(opt.camNum,1,1,1).repeat(1,grid_res_x,grid_res_y,grid_res_z).long()
            ...
            iterNum = 7500
            if grid_res_z == opt.gridsize:
                iterNum = 15000
            while (loss < average - tolerance / 2 or i < iterNum):
                average = loss
                ...
                if grid_res_x <= 32:
                    b = 100
                loss = 10 * image_loss + sdf_loss + Lp_loss
                ...
                #     x = out_min_index[:, :, :, 0].type(torch.cuda.LongTensor)
                #     y = out_min_index[:, :, :, 1].type(torch.cuda.LongTensor)
                #     z = out_min_index[:, :, :, 2].type(torch.cuda.LongTensor)
                #
                ...
                #         newpoints = cameras_bs.transform_points_screen(points=torch.stack((xw,yw,zw),dim=-1).reshape(opt.camNum,-1,3), image_size=np.tile(
                ...
                #         mask_outside_index = (seg1Batch[bs_coord,torch.zeros_like(new_y).long(),new_y,new_x]==0).long()
                ...
                # watch_grad_total = grid_initial.grad.cpu().numpy()
                # if torch.sum((x==0).float() * (y==0).float() * (z==0).float()).item() == 0:
                #     grid_initial.grad[0,0,0] = a

                # --------------------------------------------------------------------------------------------------
                # the second enhanced mask loss!
                if grid_res_z == opt.gridsize and opt.maskloss > 0:
                    grid_initial.grad = torch.where((grid_initial.data <= 0) & ( torch.logical_not(grid_mask_gt)),
                                                    - opt.maskloss * torch.ones_like(grid_initial), grid_initial.grad)
                    grid_initial.grad = torch.where((grid_initial.data > 0) & grid_mask_gt,
                                                    opt.maskloss * torch.ones_like(grid_initial), grid_initial.grad)
                    watch_grad_total = grid_initial.grad.cpu().numpy()
            ...
            image_initial, attmask, mask, inter_min_index, fine_pos = generate_image_bs(bounding_box_min_x, bounding_box_min_y,
                                                                              ...
                                                                              opt.eta2, envBatch.permute(0, 2, 3, 1),
                                                                              opt.envHeight, opt.envWidth)
            image_initial = (torch.clamp(image_initial, 0, 1)).data.permute(0, 3, 1, 2)
            torchvision.utils.save_image(image_initial,
